...
/// Scrolling that slows down quickly and comes to rest on a slot boundary, so
/// a flick lands on a page rather than between two.
class MarqueeSnapPhysics extends ScrollPhysics {
  const MarqueeSnapPhysics({super.parent, this.origin = 0});

  /// The scroll offset at which a slot sits exactly in the centre. A finite
  /// marquee is padded at both ends, so its boundaries do not fall on whole
  /// multiples of the slot height.
  final double origin;
  ...
  MarqueeSnapPhysics applyTo(ScrollPhysics? ancestor) {
    return MarqueeSnapPhysics(parent: buildParent(ancestor), origin: origin);
  }
  ...
  double restingPoint(ScrollMetrics position, double velocity) {
    ...
    final snapped =
        ((coasted - origin) / kMarqueeItemHeight).roundToDouble() *
                kMarqueeItemHeight +
            origin;
