...
Widget _app({VoidCallback? onVideoCall, VoidCallback? onVoiceCall}) =>
    MaterialApp(
...
void main() {
  testWidgets('tapping the plus opens, tapping it again closes', (
    tester,
  ) async {
  ...
  testWidgets('voice leads the video circle by the stagger when opening', (
    tester,
  ) async {
    await pumpScreen(tester, _app());
    await tester.tap(_glyph(FeatherGlyph.plus));
    ...
    expect(
      _voiceDrive(tester),
      greaterThan(0.15),
      reason: 'voice is already moving',
    );
    ...
    expect(
      _videoDrive(tester),
      greaterThan(0),
      reason: 'video starts after the stagger',
    );
    expect(_voiceDrive(tester), greaterThan(_videoDrive(tester)));
  });

  testWidgets('video leads the voice circle by the stagger when closing', (
    tester,
  ) async {
    await pumpScreen(tester, _app());
    await tester.tap(_glyph(FeatherGlyph.plus));
    ...
    expect(
      _voiceDrive(tester),
      closeTo(1, 0.001),
      reason: 'voice waits out the stagger',
    );
    expect(
      _videoDrive(tester),
      lessThan(0.95),
      reason: 'video is already retracting',
    );
  });

  testWidgets('picking an action closes the fab and reports the choice', (
    tester,
  ) async {
    var voiceCalls = 0;
  ...
  testWidgets('tapping outside closes the fab', (tester) async {
    ...
    expect(
      _voiceDrive(tester),
      0,
      reason: 'the backdrop is inert while closed',
    );
  ...
  testWidgets('closing part way through an open picks up from where it is', (
    tester,
  ) async {
    await pumpScreen(tester, _app());
  ...
  testWidgets('a stagger lets the animation it interrupts finish out the delay', (
    tester,
  ) async {
    await pumpScreen(tester, _app());
  ...
  testWidgets('the open spring overshoots past its target', (tester) async {
    ...
    expect(
      peak,
      greaterThan(1.05),
      reason: 'the looser video spring overshoots further',
    );
