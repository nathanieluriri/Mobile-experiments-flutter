...
class GooCirclesPainter extends CustomPainter {
  const GooCirclesPainter({
    required this.voiceCenterY,
    required this.videoCenterY,
  });
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
      Offset(fabCenterX, videoCenterY),
      actionDiameter / 2,
      paint,
    );
    canvas.drawCircle(
      Offset(fabCenterX, voiceCenterY),
      actionDiameter / 2,
      paint,
    );
    canvas.drawCircle(
      const Offset(fabCenterX, fabCenterY),
      fabDiameter / 2,
      paint,
    );
  }
  ...
  bool shouldRepaint(GooCirclesPainter oldDelegate) =>
      oldDelegate.voiceCenterY != voiceCenterY ||
      oldDelegate.videoCenterY != videoCenterY;
}
