...
void main() {
  ...
  group('the compact latch', () {
    test('stays open near the top of a list', () {
      final controller = TabBarController();
      addTearDown(controller.dispose);
      controller.handleScroll('browse', 0, 1000);
    ...
    test('scrolling back to the top always opens it', () {
      ...
      addTearDown(controller.dispose);
      controller.handleScroll('browse', 28, 1000);
      controller.handleScroll('browse', 400, 1000);
      expect(controller.compact, isTrue);
      controller.handleScroll('browse', 10, 1000);
      expect(controller.compact, isFalse);
    });

    test('each list keeps its own anchor', () {
      final controller = TabBarController();
      addTearDown(controller.dispose);
      controller.handleScroll('browse', 28, 1000);
      controller.handleScroll('browse', 400, 1000);
      expect(controller.compact, isTrue);
      controller.handleScroll('library', 100, 1000);
      expect(
        controller.compact,
        isTrue,
